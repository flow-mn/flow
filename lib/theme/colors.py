...
class FlowColorScheme:
    def __init__(self, primaryColor: tuple[float, float, float], accentColor: tuple[float, float, float], onAccentColor: tuple[float, float, float], onPrimaryColor: tuple[float, float, float], contrast: float, name="undefined"):
        self.name = name
...
with open("color_themes/light.dart", "w") as output_file:
    print("// Auto-generated by colors.py", file=output_file)
    print("", file=output_file)
    print('import "package:flutter/material.dart";', file=output_file)
    print('import "package:flow/theme/flow_color_scheme.dart";', file=output_file)
    print("", file=output_file)
    ...
    for [color, contrast] in generateColors(bg, 1.0, saturation=0.2, initialBrightness=1.0, totalTrials=0, hueOffset=0.80196078431):
        ...
        flowColor = FlowColorScheme(
            secondaryColor, accentColor, color, bg, secondaryContrast)

        secondaryRgb = rgbToHex(*secondaryColor)

        apiName = requests.get(
            f"https://api.color.pizza/v1/?values={secondaryRgb}").json()["colors"][0]["name"]
        flowColor.setName(to_camel_case(apiName))

        print(flowColor.toDart(), file=output_file)
